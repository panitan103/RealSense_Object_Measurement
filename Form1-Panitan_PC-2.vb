...
Public Class Form1
    ...
    Private size_obj As New List(Of Tuple(Of Double, Double))
    ...
    Private Sub Measurement_process()
        ...
        If btn_EdgeDetect.BackColor = System.Drawing.Color.Green Then
            ...
            size_obj = New List(Of Tuple(Of Double, Double))
            'For i As Integer = 0 To ctn.Size - 1
            Dim rect_crop As Rectangle = CvInvoke.BoundingRectangle(ctn(largestContourIndex))
            ...
            If btn_measurement_mode.BackColor = System.Drawing.Color.LimeGreen Then
                distance_x = (System.Math.Abs(rect.X - rect.Width) * beta_1_x) + beta_0_x
                distance_Y = (System.Math.Abs(rect.Y - rect.Height) * beta_1_y) + beta_0_y
            ElseIf btn_measurement_mode.BackColor = System.Drawing.Color.RoyalBlue Then
                distance_x = (System.Math.Abs(rect.X - rect.Width) / pixel_per_matric_x)
                distance_Y = (System.Math.Abs(rect.Y - rect.Height) / pixel_per_matric_y)
            End If


            CvInvoke.Rectangle(Edge_out, rect, New MCvScalar(0, 255, 0), 2)
            CvInvoke.PutText(Edge_out, distance_x, New Point(rect.X, rect.Y + (rect.Height / 2)), FontFace.HersheyDuplex, 0.75, New MCvScalar(255, 0, 0))
            CvInvoke.PutText(Edge_out, distance_Y, New Point(rect.X + (rect.Width / 2), rect.Y), FontFace.HersheyDuplex, 0.75, New MCvScalar(255, 0, 0))

            CvInvoke.Rectangle(img_filtered, rect, New MCvScalar(0, 255, 0), 2)
            CvInvoke.PutText(img_filtered, distance_x, New Point(rect.X, rect.Y + (rect.Height / 2)), FontFace.HersheyDuplex, 0.75, New MCvScalar(255, 0, 0))
            CvInvoke.PutText(img_filtered, distance_Y, New Point(rect.X + (rect.Width / 2), rect.Y), FontFace.HersheyDuplex, 0.75, New MCvScalar(255, 0, 0))

            size_obj.Add(Tuple.Create(distance_x, distance_Y))
    ...
    Sub find_pixel_per_matric()
        ...
        Dim X_real As New List(Of Double)({0, maker_x / 2, maker_x, maker_x, maker_x, maker_x / 2, 0, 0})
        Dim Y_real As New List(Of Double)({0, 0, 0, maker_y / 2, maker_y, maker_y, maker_y, maker_y / 2})
        pixel_per_matric_x = (Aruco_coordinate.Item(2).Item1 - Aruco_coordinate.Item(0).Item1) / maker_x
        pixel_per_matric_y = (Aruco_coordinate.Item(6).Item2 - Aruco_coordinate.Item(0).Item2) / maker_y

        Debug.Print("TL" & " = " & Aruco_coordinate.Item(0).Item1 & vbCrLf &
                    "TR" & " = " & Aruco_coordinate.Item(2).Item1 & vbCrLf &
                    "BL" & " = " & Aruco_coordinate.Item(6).Item1 & vbCrLf &
                    "TL to TR" & " = " & (Aruco_coordinate.Item(2).Item1 - Aruco_coordinate.Item(0).Item1) & vbCrLf &
                    "TL to BL" & " = " & (Aruco_coordinate.Item(6).Item1 - Aruco_coordinate.Item(0).Item1) & vbCrLf &
                    "pixel per metric x" & " = " & pixel_per_matric_x)
